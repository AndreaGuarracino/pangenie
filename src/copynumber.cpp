...
double CopyNumber::get_probability_of(int cn) const {
	...
	if( (cn == 2) && (this->probabilities.size() < 3)) {
		return 1.0 - probabilities.at(0) - probabilities.at(1);
	} else {
...
bool CopyNumber::operator==(const CopyNumber &other) const{
	vector<double> p1 = this->probabilities;
	if (p1.size() < 3) p1.push_back(1.0 - p1[0] - p1[1]);
	vector<double> p2 = other.probabilities;
	if (p2.size() < 3) p1.push_back(1.0 - p2[0] - p2[1]);

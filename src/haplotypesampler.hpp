...
public:
	...
	**/
	HaplotypeSampler(std::vector<std::shared_ptr<UniqueKmers>>* unique_kmers, size_t size, double recombrate = 1.26, double effective_N = 25000.0, std::vector<unsigned int>* best_scores = nullptr, bool add_reference = false, std::string path_output="", std::string chromosome = "None", unsigned short allele_penalty = 10, double* time = nullptr );
